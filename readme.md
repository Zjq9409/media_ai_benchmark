...
## 5. tune_local_streams.sh 高级参数说明
...
| `-g <device>` | 使用的 GPU 设备（GPU.0, GPU.1 等） |
...
```bash
# 自定义 5 个并发流，不分组
./tune_local_streams.sh -f ./video/1280x720_25fps.h265 -m ./yolo11n_seg/ -n 5

# Oneshot 模式：固定 10 个流运行一次
./tune_local_streams.sh -f ./video/1280x720_25fps.h265 -m ./yolo11n_seg/ -n 10 -o
```
