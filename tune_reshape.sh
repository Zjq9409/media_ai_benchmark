...
generate_group_script() {
    ...
    # Optional reshape support via env vars RESHAPE_WIDTH / RESHAPE_HEIGHT
    local reshape_args=""
    if [[ -n "${RESHAPE_WIDTH:-}" && -n "${RESHAPE_HEIGHT:-}" ]]; then
        reshape_args=" reshape=true reshape-width=${RESHAPE_WIDTH} reshape-height=${RESHAPE_HEIGHT}"
    fi
    ...
    for ((j=1; j<=streams_in_group; j++)); do
        ...
        if [[ -n "${NO_VPP:-}" ]]; then
            pipeline="$pipeline $parse_element ! $decode_element !"
        else
        ...
        if [[ "$MODE" == "YOLO" ]]; then
            pipeline="$pipeline gvadetect model=\"$MODEL_XML\" $MODEL_PROC device=$GPU_DEVICE pre-process-backend=va-surface-sharing scale-method=default batch-size=$BATCH_SIZE${reshape_args} model-instance-id=yolo-${group_id} !"
